...
if you want to test directly, please do not run 'python rnn_pretrain.py ...' but run 'python rnn_finetune.py ... --is_finetune 0' instead ('...' represents the context of the above command). This will test the model through the prepared ckpts in '/saved_models' (ignore the patchtst in the name of ckpts, that is RNN in practice)
