...
def find_lr(head_type):
    ...
    # get callbacks
    cbs = [RevInCB(dls.vars, affine=True)] if args.revin else []
...
def finetune_func(lr=args.lr):
    ...
    loss_func = torch.nn.MSELoss(reduction='mean')   
    # get callbacks
    cbs = [RevInCB(dls.vars, affine=True, denorm=True)] if args.revin else []
    cbs += [
...
def linear_probe_func(lr=args.lr):
    ...
    # transfer weight
    weight_path = args.save_path + args.pretrained_model + '.pth'
    model = transfer_weights(weight_path, model)  # args.pretrained_model
    # get loss
    loss_func = torch.nn.MSELoss(reduction='mean')    
    # get callbacks
